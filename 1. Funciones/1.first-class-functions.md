...
Una de las razones por las que las funciones y los conceptos funcionales son tan importantes en JavaScript es por que las funciones son las unidades primarias de ejecución. Execepto por la ejecución de JavaScript en el contexto global en la fase de construcción de nuestra página, todo el código que ejecutaremos en nuestra aplicación será mediante funciones.

Debido a que la mayoría de nuestro código será ejecutado como resultado de la invocación de una función, tener funciones versátiles y poderosas nos dan una gran flexibilidad y balance a la hora de escribir nuestro código.
...
```js
var funcion = function() {}; // Asignadas a una variable.
var arr = [];
...
Cualquier cosa que podamos hacer con objetos lo podemos hacer con funciones también. Las funciones son objetos con funcionalidades adicionales y con la capacida de ser invocadas, es decir, las funciones pueden ser invocadas para ejecutar una acción y los objetos no.

Una de las características más importantes de las funciones de primera clase es la de ser pasadas como argumentos a otras funciones. Ya que esto nos permite ejecutar las funciones que pasemos como argumentos en algún punto durante la ejecución de la función principal. Este concepto es más conocido como funciones callback.
